...
implementation{
    task void processCommand(){
        if(! call Queue.empty()){
            ...
            case CMD_APP_CLIENT:
                dbg(COMMAND_CHANNEL, "Command Type: App Client Setup\n");
                signal CommandHandler.setAppClient(buff[0], &buff[1]);
                break;

            case CMD_APP_SERVER:
                dbg(COMMAND_CHANNEL, "Command Type: Server Client Setup\n");
